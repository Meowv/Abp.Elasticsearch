...
namespace Abp.Elasticsearch
{
    public class AbpElasticsearch : IElasticsearch
    {
        ...
        {
            if (ElasticsearchOptions.Nodes.Count == 0)
                throw new ElasticSearchException("Elasticsearch 连接字符串配置有误");

            ConnectionSettings connectionSettings;

            if (ElasticsearchOptions.Nodes.Count > 1)
            {
                var nodes = ElasticsearchOptions.Nodes.Select(s => new Uri(s)).ToList();
            ...
            {
                var node = ElasticsearchOptions.Nodes.Select(s => new Uri(s)).FirstOrDefault();
        ...
        {
            var exits = await ElasticClient.Indices.AliasExistsAsync(indexName);
            ...
            {
                await ElasticClient.Indices.PutAliasAsync(newName, indexName);
                return;
            }
        ...
        {
            var exits = ElasticClient.DocumentExists(DocumentPath<T>.Id(new Id(model)), dd => dd.Index(indexName));
        ...
        {
            var result = await ElasticClient.Indices.GetAliasAsync(indexName);
            var oldName = result.Indices.Keys.First();
            //创建新的索引
            var newIndex = indexName + DateTime.Now.Ticks;
            var createResult = await ElasticClient.Indices.CreateAsync(newIndex,
                c =>
                    c.Index(newIndex)
                        .Map(m => m.AutoMap<T>()));
            ...
            //重建索引数据
            var reResult = await ElasticClient.ReindexOnServerAsync(descriptor => descriptor
            ...
            //删除旧索引
            var deleteResult = await ElasticClient.Indices.DeleteAsync(oldName);
            var reAliasResult = await ElasticClient.Indices.PutAliasAsync(newIndex, indexName);
